...
public class Inbook extends Entry {
	private String author, editor, title, chapter, pages, publisher, year, volume, number, series, address, edition, month, note, crossref;
	...
	public String[] printContents() {
		...
		contents[0] += " <i>" + title + "</i>,";
		if (volume != null && series != null) contents[0] += " volume " + volume.substring(1, volume.length()-1) + " of " + series.substring(1, series.length()-1) + ",";
		if (number != null && series != null) contents[0] += " number " + number.substring(1, number.length()-1) + " of " + series.substring(1, series.length()-1) + ",";
